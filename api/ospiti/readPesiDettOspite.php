...
if ($jwt) {
    ...
    try {
        ...
        if ($jwt->isValid()) {
            ...
            $arrayTmp = $Ospiti->GetPesiDettOspite($data->Schema, 'vRiepilogoPesi', $idOspite);
            ...
            echo json_encode(array(
                "message" => "Token scaduto, verrai reindirizzato alla pagina di Login (readPesiDettOspite.php)",
                "error" => "TOKEN SCADUTO"
            ));
        }
    } catch (Exception $e) {
        ...
        echo json_encode(array(
            "message" => "Access denied. Eccezione interna (readPesiDettOspite.php)",
            "error" => $e->getMessage()
        ));
    }
}    